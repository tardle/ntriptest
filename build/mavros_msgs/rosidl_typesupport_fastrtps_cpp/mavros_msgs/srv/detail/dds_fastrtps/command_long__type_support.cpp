  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = mavros_msgs::srv::CommandLong_Request;
    is_plain =
      (
      offsetof(DataType, param7) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _CommandLong_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_CommandLong_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = mavros_msgs::srv::CommandLong_Response;
    is_plain =
      (
...
static size_t _CommandLong_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_CommandLong_Response(full_bounded, is_plain, 0);
