  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = mavros_msgs::srv::MountConfigure_Request;
    is_plain =
      (
      offsetof(DataType, yaw_input) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _MountConfigure_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_MountConfigure_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = mavros_msgs::srv::MountConfigure_Response;
    is_plain =
      (
...
static size_t _MountConfigure_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_MountConfigure_Response(full_bounded, is_plain, 0);
