  ...
  eprosima::fastcdr::Cdr & cdr)
{
  ...
  // Member: payload64
  {
    ...
    if (size > 0) {
      cdr.serializeArray(&(ros_message.payload64[0]), size);
    }
  }
  // Member: signature
  {
    ...
    cdr << static_cast<uint32_t>(size);
    if (size > 0) {
      cdr.serializeArray(&(ros_message.signature[0]), size);
    }
  }
  return true;
}
  ...
  mavros_msgs::msg::Mavlink & ros_message)
{
  ...
  // Member: payload64
  {
    ...
    if (size > 0) {
      cdr.deserializeArray(&(ros_message.payload64[0]), size);
    }
  }
  ...
  // Member: signature
  {
    ...
    if (size > 0) {
      cdr.deserializeArray(&(ros_message.signature[0]), size);
    }
  }
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = mavros_msgs::msg::Mavlink;
    is_plain =
      (
      offsetof(DataType, signature) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _Mavlink__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_Mavlink(full_bounded, is_plain, 0);
