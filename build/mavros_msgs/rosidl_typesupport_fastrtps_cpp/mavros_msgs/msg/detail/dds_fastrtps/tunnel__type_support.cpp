  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = mavros_msgs::msg::Tunnel;
    is_plain =
      (
...
static size_t _Tunnel__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_Tunnel(full_bounded, is_plain, 0);
