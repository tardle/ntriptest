  ...
  void * untyped_ros_message)
{
  ...
  // Field name: esc_status
  {
    ...
    if (!mavros_msgs__msg__ESCStatusItem__Sequence__init(&ros_message->esc_status, size)) {
      fprintf(stderr, "failed to create array for field 'esc_status'");
      return false;
    }
    auto array_ptr = ros_message->esc_status.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = mavros_msgs__msg__ESCStatus;
    is_plain =
      (
...
static size_t _ESCStatus__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_mavros_msgs__msg__ESCStatus(
