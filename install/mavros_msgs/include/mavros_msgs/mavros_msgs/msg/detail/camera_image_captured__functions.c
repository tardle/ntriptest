// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from mavros_msgs:msg/CameraImageCaptured.idl
// generated code does not contain a copyright notice
#include "mavros_msgs/msg/detail/camera_image_captured__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"
// Member `orientation`
#include "geometry_msgs/msg/detail/quaternion__functions.h"
// Member `geo`
#include "geographic_msgs/msg/detail/geo_point__functions.h"
// Member `file_url`
#include "rosidl_runtime_c/string_functions.h"

bool
mavros_msgs__msg__CameraImageCaptured__init(mavros_msgs__msg__CameraImageCaptured * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    mavros_msgs__msg__CameraImageCaptured__fini(msg);
    return false;
  }
  // orientation
  if (!geometry_msgs__msg__Quaternion__init(&msg->orientation)) {
    mavros_msgs__msg__CameraImageCaptured__fini(msg);
    return false;
  }
  // geo
  if (!geographic_msgs__msg__GeoPoint__init(&msg->geo)) {
    mavros_msgs__msg__CameraImageCaptured__fini(msg);
    return false;
  }
  // relative_alt
  // image_index
  // capture_result
  // file_url
  if (!rosidl_runtime_c__String__init(&msg->file_url)) {
    mavros_msgs__msg__CameraImageCaptured__fini(msg);
    return false;
  }
  return true;
}

void
mavros_msgs__msg__CameraImageCaptured__fini(mavros_msgs__msg__CameraImageCaptured * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // orientation
  geometry_msgs__msg__Quaternion__fini(&msg->orientation);
  // geo
  geographic_msgs__msg__GeoPoint__fini(&msg->geo);
  // relative_alt
  // image_index
  // capture_result
  // file_url
  rosidl_runtime_c__String__fini(&msg->file_url);
}

bool
mavros_msgs__msg__CameraImageCaptured__are_equal(const mavros_msgs__msg__CameraImageCaptured * lhs, const mavros_msgs__msg__CameraImageCaptured * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // orientation
  if (!geometry_msgs__msg__Quaternion__are_equal(
      &(lhs->orientation), &(rhs->orientation)))
  {
    return false;
  }
  // geo
  if (!geographic_msgs__msg__GeoPoint__are_equal(
      &(lhs->geo), &(rhs->geo)))
  {
    return false;
  }
  // relative_alt
  if (lhs->relative_alt != rhs->relative_alt) {
    return false;
  }
  // image_index
  if (lhs->image_index != rhs->image_index) {
    return false;
  }
  // capture_result
  if (lhs->capture_result != rhs->capture_result) {
    return false;
  }
  // file_url
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->file_url), &(rhs->file_url)))
  {
    return false;
  }
  return true;
}

bool
mavros_msgs__msg__CameraImageCaptured__copy(
  const mavros_msgs__msg__CameraImageCaptured * input,
  mavros_msgs__msg__CameraImageCaptured * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // orientation
  if (!geometry_msgs__msg__Quaternion__copy(
      &(input->orientation), &(output->orientation)))
  {
    return false;
  }
  // geo
  if (!geographic_msgs__msg__GeoPoint__copy(
      &(input->geo), &(output->geo)))
  {
    return false;
  }
  // relative_alt
  output->relative_alt = input->relative_alt;
  // image_index
  output->image_index = input->image_index;
  // capture_result
  output->capture_result = input->capture_result;
  // file_url
  if (!rosidl_runtime_c__String__copy(
      &(input->file_url), &(output->file_url)))
  {
    return false;
  }
  return true;
}

mavros_msgs__msg__CameraImageCaptured *
mavros_msgs__msg__CameraImageCaptured__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mavros_msgs__msg__CameraImageCaptured * msg = (mavros_msgs__msg__CameraImageCaptured *)allocator.allocate(sizeof(mavros_msgs__msg__CameraImageCaptured), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(mavros_msgs__msg__CameraImageCaptured));
  bool success = mavros_msgs__msg__CameraImageCaptured__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
mavros_msgs__msg__CameraImageCaptured__destroy(mavros_msgs__msg__CameraImageCaptured * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    mavros_msgs__msg__CameraImageCaptured__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
mavros_msgs__msg__CameraImageCaptured__Sequence__init(mavros_msgs__msg__CameraImageCaptured__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mavros_msgs__msg__CameraImageCaptured * data = NULL;

  if (size) {
    data = (mavros_msgs__msg__CameraImageCaptured *)allocator.zero_allocate(size, sizeof(mavros_msgs__msg__CameraImageCaptured), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = mavros_msgs__msg__CameraImageCaptured__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        mavros_msgs__msg__CameraImageCaptured__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
mavros_msgs__msg__CameraImageCaptured__Sequence__fini(mavros_msgs__msg__CameraImageCaptured__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      mavros_msgs__msg__CameraImageCaptured__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

mavros_msgs__msg__CameraImageCaptured__Sequence *
mavros_msgs__msg__CameraImageCaptured__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  mavros_msgs__msg__CameraImageCaptured__Sequence * array = (mavros_msgs__msg__CameraImageCaptured__Sequence *)allocator.allocate(sizeof(mavros_msgs__msg__CameraImageCaptured__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = mavros_msgs__msg__CameraImageCaptured__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
mavros_msgs__msg__CameraImageCaptured__Sequence__destroy(mavros_msgs__msg__CameraImageCaptured__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    mavros_msgs__msg__CameraImageCaptured__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
mavros_msgs__msg__CameraImageCaptured__Sequence__are_equal(const mavros_msgs__msg__CameraImageCaptured__Sequence * lhs, const mavros_msgs__msg__CameraImageCaptured__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!mavros_msgs__msg__CameraImageCaptured__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
mavros_msgs__msg__CameraImageCaptured__Sequence__copy(
  const mavros_msgs__msg__CameraImageCaptured__Sequence * input,
  mavros_msgs__msg__CameraImageCaptured__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(mavros_msgs__msg__CameraImageCaptured);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    mavros_msgs__msg__CameraImageCaptured * data =
      (mavros_msgs__msg__CameraImageCaptured *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mavros_msgs__msg__CameraImageCaptured__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          mavros_msgs__msg__CameraImageCaptured__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!mavros_msgs__msg__CameraImageCaptured__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
